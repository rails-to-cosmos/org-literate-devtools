...
(defvar oldt-jira-url-issue "https://flocktory.atlassian.net/rest/api/latest/issue/")

(defun oldt-jira-auth-header ()
  `(("Authorization" . ,(oldt-jira-get-auth-token))))

(defun oldt-jira-url-ticket-data (ticket &optional parameter)
  (concat
   oldt-jira-url-issue
   ticket
   (if parameter
...
(defun oldt-jira-handler (method)
  ...
  (case method
    ...
    ('ticket-worklog-read (cl-function
    ...
    ('ticket-title-capture (cl-function
                            (lambda (&key data &allow-other-keys)
                              (save-window-excursion
                                (save-excursion
                                  (let-alist data
...
(defun oldt-jira-get-ticket-worklog (ticket)
  (request
    (oldt-jira-url-ticket-data ticket "worklog")
    :headers (oldt-jira-auth-header)
    :parser 'json-read
    :success (oldt-jira-handler 'ticket-worklog-read)))
...
(defun oldt-jira-capture-ticket-title (&optional ticket)
  (when-let (ticket (or ticket (oldt-project-get-property "TICKET")))
    (oldt-jira-get-ticket-summary ticket (oldt-jira-response-handle 'ticket-title-capture))))
